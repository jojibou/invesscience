...
def clean_training_data(companies, reference="a"):
    ...
    filled_1 = pd.read_csv("../raw_data/datanamed_completed.csv", sep=";", header=1)[["id","state_code"]].set_index("id")
    filled_2 = pd.read_csv("../raw_data/countries_filled.csv", sep=";")[["id","country_code"]].set_index("id")
    filled_3 = pd.read_csv("../raw_data/categories_filled.csv", sep=";")[["id","category_code"]].set_index("id")
    ...
    companies[f"date_series_{reference}"] = companies[f"date_series_{reference}"].map(lambda x: np.nan if pd.isnull(x) else x).dt.strftime('%Y').map(clean_date_series)

    # cleaning participants series reference, 5+ =5
    companies[f"participants_{reference}"] = companies[f"participants_{reference}"].map(clean_participants)

    # cleaning amount series reference in 7 buckets and removing 0s
    companies[f"raised_amount_usd_{reference}"] = companies[f"raised_amount_usd_{reference}"].map(clean_amount)

    # cleaning time to series reference in 3 buckets, becomes categorical
    companies[f"timediff_founded_series_{reference}"]= companies[f"timediff_founded_series_{reference}"].map(clean_time)
    ...
    companies["top_20_bool"] = (companies["top_5_bool"] + companies["top_20_bool"]).map(clean_school)

    #graduate and undergrad as a bool
    companies["graduate"] = companies.graduate.map(clean_study)
    companies["undergrad"] = companies.undergrad.map(clean_study)
    companies["professional"] = companies.undergrad.map(clean_study)

    # degree count 4 categories 0 - 3
    companies["degree_count"] = companies.degree_count.map(clean_study)

    #companies founded before to bool
    companies["mean_comp_founded_before"] = companies["mean_comp_founded_before"].map(clean_founded_before)

    #companies worked before to 0 - 1 - 2
    companies["mean_comp_worked_before"] = companies["mean_comp_worked_before"].map(clean_worked_before)
